...
| `clozetext`       | "fill in the blank", "cloze with typed answers"                     | Inline blanks filled by typing; one or more blanks per stem. Say so if an answer runs long — blanks cap typed input at 15 characters unless raised. |
...
| `token-highlight` | "token highlight", "hot text", "highlight the words", "click the verbs", "select the words in the text" | Highlight tokens in a passage. List the correct tokens and the clickable distractors; only listed tokens are clickable. |
...
- **Token highlight (also called "hot text")** — the learner clicks words/phrases in a passage. Provide the passage, the correct tokens, and the *distractors* (clickable words that are wrong). Only the tokens you list are clickable. Matching is case-insensitive and whole-word, and a repeated correct word counts at every occurrence; say "they may pick at most N" to cap selections. Works for tasks like "click every verb" or "highlight the supporting evidence".
